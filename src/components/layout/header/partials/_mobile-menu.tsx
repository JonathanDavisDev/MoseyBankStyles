...
export default function MobileMenu() {
  const { currentMenu, setCurrentMenu, menuItems, mobileMenuOpen } = useContext(HeaderContext);
  ...
  return (
    <section className="absolute pt-10 pb-20 z-50 top-[88px] left-0 bg-ghost-white w-full outer-padding shadow-[0_14px_4px_6px_rgba(0,0,0,0.1)]">
      <div className="container mx-auto grid">
        <ul>
          {menuItems.map(({ menuName, menuData }: DropdownMenuProps) => (
            <li key={menuName}>
              <button
                className={`hover:text-azure focus:text-azure font-semibold text-[18px] py-6 text-left border-t mt-[-1px] w-full flex justify-between ${currentMenu === menuName ? "" : "border-b"}`}
              ...
              {currentMenu === menuName && (
                <ul className="mb-16">
                  {menuData?.menuLists.map(({ title, items }: MenuList) => (
                    <li key={title} className="mt-10">
                      {title ? <h3 className="text-[16px] font-semibold uppercase tracking-[1px]">{title}</h3> : null}
